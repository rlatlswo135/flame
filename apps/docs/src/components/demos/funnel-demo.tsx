...
const StepIndicator = ({
	...
	<div className="flex items-center gap-2 mb-4">
		{Array.from({ length: total }, (_, i) => (
			<div
				// biome-ignore lint/suspicious/noArrayIndexKey: fixed-length static step indicator, items never reorder
				key={i}
...
function FunnelDemo({ t }: { t: Record<string, string> }) {
	return (
						...
						<Funnel.Step key={key}>
							{({ jump }) => (
								<div>
									...
									<h3 className="text-lg font-semibold text-flame-text mb-1">
										{t[key]}
									</h3>
